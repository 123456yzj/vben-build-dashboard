      ...
      <template v-else>
        ...
        <div class="repository-list">
          <div v-for="repo in repositories" :key="repo.name" class="repository-row">
            ...
            <div class="repository-status"><span><GitBranch :size="14" />{{ repo.git?.branch || '不可用' }}</span><span :class="repo.error ? 'text-error' : repo.git?.clean ? 'text-good' : 'text-warn'">{{ repo.error ? '读取失败' : repo.git?.clean ? 'clean' : 'dirty' }}</span><small v-if="repo.error" :title="repo.error">{{ repo.error }}</small></div>
