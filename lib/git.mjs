...
export async function runGit(cwd, args, options = {}) {
  ...
  try {
    const { stdout, stderr } = await execAsync(cmd, { cwd, maxBuffer: 10 * 1024 * 1024, ...options });
...
export async function getRepoStatus(repoPath, name, packageName = '') {
  ...
  const dirtyFiles = dirtyLines.slice(0, 15); // first 15 files summary
...
export async function scanAllRepos(targetRepoPath) {
  ...
  // 1. Root Repo
  const rootStatus = await getRepoStatus(targetRepoPath, 'root (shared)', rootPkgName);
  repos.push({ ...rootStatus, isRoot: true });
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        repos.push({ ...appStatus, isRoot: false });
      }
    }
  } catch (err) {
...
export async function batchCheckoutAll(targetRepoPath, branchName) {
  ...
  for (const repo of allRepos) {
    ...
    // Fetch first to get fresh remote refs
